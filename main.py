...
def main():
    graph_file = 'test_graphs/cliques_8x12_bridges.col'
    problem_graph = parse_problem_data_text_to_nx_graph(graph_file)

    print(len(problem_graph.nodes))

    colors, score = colony.optimize(problem_graph, num_ants=50, num_iterations=100, heuristic_weight=4.0, pheromone_weight=2.0, evaporate_value=0.5, threads=16)
